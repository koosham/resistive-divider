...
function findResistorPairs(ratio, pool)
{
    var results = Array();
    
    pool.forEach(function(r1) {
        pool.forEach(function(r2) {
            var res_exp = toScientific(r2 / r1);
            var ratio_exp = toScientific(ratio);
            var error = (res_exp.mantissa - ratio_exp.mantissa) / ratio_exp.mantissa;

            if(Math.abs(error) < 0.2)
            {
                var order = Math.pow(10, (ratio_exp.exponent - res_exp.exponent));
                results.push({
...
function clearResults(destId)
{
    var results = document.getElementById(destId);
...
function show(destId, resistors)
{
    var results = document.getElementById(destId);
...
{
    var ss = document.getElementById(seriesId);
    var s = ss.options[ss.selectedIndex].value;
    var vi = Number(document.getElementById(viId).value);
    var vo = Number(document.getElementById(voId).value);
    
    if(vi > 0.0 && vo > 0.0 && vo < vi)
    {
        var ratio = vo / (vi - vo); // (r2 / r1)
        var resistorPairs = findResistorPairs(ratio, E_SERIES_RESISTORS[s]);
        clearResults(destId);
        resistorPairs.forEach(function(p) {
            show(destId, p)
        });
    }
    else
